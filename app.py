...
def generate_sql(state: AgentState):
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an expert SQLite data analyst. Write a raw SQL query. Return ONLY SQL. "
                   ...
                   "1. NEVER use arbitrary table aliases like T1, T2. ALWAYS write out the full table name. "
                   ...
                   "4. If asked for an 'overall' metric (e.g. 'average across all'), do NOT use GROUP BY. But if asked for 'Top 5' or a breakdown, you MUST use GROUP BY. "
                   ...
                   "8. For city/state filters, use 'customer_city'/'seller_city' directly from the customers/sellers tables. Do NOT use the geolocation table. "
                   "9. City names in the database are lowercase. Always use LOWER() for text matching: e.g., LOWER(sellers.seller_city) = 'sao paulo'."
